...
import { currentMonth, monthRange, yearRange } from '../../../utils/month';
...
export default function PaymentsBody() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['partner-payments', partnerId, propertyId, range.from, range.to, page],
    queryFn: () =>
      fetchPartnerPayments(partnerId, propertyId, range.from, range.to, page, PAGE_SIZE, token!),
    enabled,
  });
  ...
  return (
    <PageContainer>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, gap: 1.5, mb: 2 }}>
        <MetricCard
          label={t('partner.payments.kpi.gross')}
          value={data ? formatPrice(data.totals.gross, currency) : '—'}
          variant="positive"
          loading={isLoading}
        />
        <MetricCard
          label={t('partner.payments.kpi.commission')}
          value={data ? formatPrice(data.totals.commission, currency) : '—'}
          variant="negative"
          loading={isLoading}
        />
        <MetricCard
          label={t('partner.payments.kpi.earnings')}
          value={data ? formatPrice(data.totals.net, currency) : '—'}
          variant="positive"
          loading={isLoading}
        />
      </Box>
