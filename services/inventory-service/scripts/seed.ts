...
async function seed() {
  ...
  // ── Availability — aligned with search-service booked_ranges ─────────────
  // reserved_rooms reflects bookings already in place for manual testing.
  ...
  for (const range of bookedRanges) {
    for (const date of eachDay(range.from_date, range.to_date)) {
      await db
        ...
        .onConflict((oc) =>
          oc.columns(["room_id", "date"]).doUpdateSet((eb) => ({
            reserved_rooms: eb.ref("inv_availability.reserved_rooms"),
          })),
        )
        .execute();
    }
