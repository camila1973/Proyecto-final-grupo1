...
// __dirname resolves to dist/partners-service/src/exports/ at runtime.
...
@Injectable()
export class PaymentsPdfRenderer {
  render(data: ReportData, locale: ReportLocale, res: Response): void {
    ...
    const doc = new PDFDocument({
      size: [PAGE_W, PAGE_H],
      // Bottom margin is intentionally 0 so footer art (drawn near the page
      // bottom) doesn't trigger pdfkit's auto-pagination when its y exceeds
      // page.maxY(). The body content stays within MARGIN via explicit positioning.
...
function drawFooter(
  ...
  // lineBreak: false prevents pdfkit from auto-paginating when y is past the
  // document's bottom margin (which it is here, by design — this is footer art).
...
export function reportFilename(data: ReportData, ext: "pdf" | "csv"): string {
