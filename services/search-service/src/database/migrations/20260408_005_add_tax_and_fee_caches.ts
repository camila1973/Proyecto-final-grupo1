...
export async function up(db: Kysely<any>): Promise<void> {
  // Add tax_rate_pct column to room_search_index
  await sql`
    ALTER TABLE room_search_index
      ADD COLUMN IF NOT EXISTS tax_rate_pct NUMERIC(8,4) NOT NULL DEFAULT 0
  `.execute(db);

  // Pre-summed percentage tax rate cache for a (country, city) pair
  await sql`
  ...
  // Local cache of partner fees for search estimates
  await sql`
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await sql`
    ALTER TABLE room_search_index DROP COLUMN IF EXISTS tax_rate_pct
  `.execute(db);
}
